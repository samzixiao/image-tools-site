...
assert.ok(source.includes("const imageLayersModule") && source.indexOf("{imageLayersModule}") < source.indexOf('<section className="preview">'), "Layer controls must be placed in the right editor column");
...
assert.ok(source.includes('function frameForCropAspect(targetAspect: number)') && source.includes('setCustomFrame(frameForCropAspect(outputWidth / outputHeight));'), "Custom ratio input must redraw the visible crop frame");
assert.ok(styles.includes('.workspace .shape-icon { font-size: 23px !important;') && styles.includes('font-size: 22px !important;'), "Main editor shapes and privacy stickers must use the enlarged controls");
assert.ok(styles.includes('.workspace .controls .caption-add,') && styles.includes('font-size: 16px !important;') && styles.includes('font-weight: 700 !important;'), "Add text action must be larger and bold");
assert.ok(styles.includes('.workspace .controls .layer-section > button,\n.workspace .controls .measure-add { font-size: 15px !important; }') && styles.includes('.workspace .right-image-actions .danger-action { font-size: 14px !important; }'), "Specified add and remove actions must retain their previous size");
assert.ok(styles.includes('.polygon-preview polygon { fill: transparent; }'), "Polygon crop must use an outline without a red filled area");
