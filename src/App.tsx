...
function App() {
  ...
  function togglePreset(next: Preset) {
    const isActive = sizeSelected && preset.id === next.id;
    if (isActive) {
      setBatchPresetIds([]);
  ...
  function toggleShape(nextShape: Shape) {
    ...
    setShape((current) => current === nextShape ? "original" : nextShape);
  }
